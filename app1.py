...
class VoterPredictor:
    ...
    def preprocess_voter_data_vectorized(self, voter_rows):
        ...
        for i, r in enumerate(voter_rows):
            ...
            # Build categorical dict for DictVectorizer
            cat = {}
        ...
        if self.vectorizer is None:
            ...
            # Strict mode: raise if any NON-NUMERIC checkpoint feature cannot be mapped to vectorizer column
            missing = [
    ...
    def predict_voters_vectorized(self, voter_rows):
        ...
        Args:
            voter_rows: DataFrame, list of dicts, or single dict/Series
            
        Returns:
            List of prediction dicts with keys:
                - 'turnout_probability': float
                - 'party_probabilities': dict {party_name: probability}
                - 'predicted_party': str (party with highest probability)
        """
        ...
        for i in range(n_voters):
            ...
            party_probabilities = {
                party: float(prob) 
                for party, prob in zip(self.party_names, party_probs)
            }
            ...
            if sample:
                ...
                try:
                    ...
                    for r in sample_records_list[:5]:
                        ...
                        caste_raw = str(get_any(r, 'caste', default='')).upper()
                        caste_map = {
                        ...
                        blank_caste = caste_raw.strip() == "" or caste_raw in {"NA", "N/A", "NONE", "NULL"}
                        if blank_caste:
                            if rel_tok == "Religion_Hindu":
                                caste_tok = "Caste_Obc"
                            else:
                                caste_tok = "Caste_No_caste_system"
                        else:
