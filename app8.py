...
class VoterPredictor:
    def __init__(self):
        ...
        self.training_info = {}
        self.correction_log = []
        # Optional thresholds for alignment categories
        ...
        # cached vectorizer metadata
        self._feature_to_vec_idx = None
        self._vec_feats = None
    ...
    def load_model(self, model_file):
        ...
        self.training_info = first_present(model_data, ['training_info', 'train_info', 'training'], {}) or {}
        self.correction_log = first_present(model_data, ['correction_log', 'data_corrections', 'log'], []) or []
        # optional alignment thresholds if provided in checkpoint
        at = first_present(model_data, ['alignment_thresholds', 'alignment', 'thresholds'], None)
        if isinstance(at, dict):
            core = at.get('core') or at.get('core_threshold')
            leaning = at.get('leaning') or at.get('leaning_threshold')
            try:
                if core is not None:
                    self.alignment_thresholds['core'] = float(core)
                if leaning is not None:
                    self.alignment_thresholds['leaning'] = float(leaning)
            except Exception:
                pass
        ...
        # Build vectorizer feature mapping once for faster predictions
        try:
            self._initialize_vectorizer_metadata()
        except Exception:
            pass
        return True

    def _initialize_vectorizer_metadata(self):
        """Precompute mapping from checkpoint feature_names -> vectorizer column indices."""
        if self.vectorizer is None or not self.feature_names:
            self._feature_to_vec_idx = {}
            self._vec_feats = []
            return
        ...
        if vec_feats is None:
            vec_feats = []
        self._vec_feats = list(vec_feats)

        vec_idx_dict = {n: i for i, n in enumerate(self._vec_feats)}
        feature_to_vec_idx = {}
        # exact matches
        for fname in self.feature_names:
            if fname in vec_idx_dict:
                feature_to_vec_idx[fname] = vec_idx_dict[fname]

        # normalized matches
        if len(feature_to_vec_idx) < len(self.feature_names):
            norm_vec = {str(n).lower().strip(): i for i, n in enumerate(self._vec_feats)}
            for fname in self.feature_names:
        ...
        # If still missing and lengths match, index-align fallback
        if len(feature_to_vec_idx) < len(self.feature_names) and len(self._vec_feats) == len(self.feature_names):
            for j, fname in enumerate(self.feature_names):
                if fname not in feature_to_vec_idx:
                    feature_to_vec_idx[fname] = j
            self._used_index_alignment = True
        else:
            self._used_index_alignment = False

        self._feature_to_vec_idx = feature_to_vec_idx
    ...
    def preprocess_voter_data_vectorized(self, voter_rows):
        ...
        if self.vectorizer is None:
            ...
            if self._feature_to_vec_idx is None:
                try:
                    self._initialize_vectorizer_metadata()
                except Exception:
                    self._feature_to_vec_idx = {}
            vec_idx = self._feature_to_vec_idx or {}
    ...
    # -------------------------------
    def classify_alignment(self, prob_vector):
        """Return ('core'|'leaning'|'swing', dominant_party, max_prob)."""
        try:
            pvec = np.asarray(prob_vector, dtype=float)
            if pvec.size == 0:
                return 'swing', None, 0.0
            pmax = float(np.max(pvec))
            party = self.party_names[int(np.argmax(pvec))] if self.party_names else None
            if pmax >= self.alignment_thresholds.get('core', 0.7):
                return 'core', party, pmax
            if pmax >= self.alignment_thresholds.get('leaning', 0.4):
                return 'leaning', party, pmax
            return 'swing', party, pmax
        except Exception:
            return 'swing', None, 0.0
    ...
    def predict_voters_vectorized(self, voter_rows):
        ...
        for i in range(n):
            ...
            align_cat, align_party, pmax = self.classify_alignment(pvec)
            results.append({
                ...
                "alignment_party": align_party,
                "alignment_confidence": float(pmax)
            })
        return results
...
def load_voter_data(file_bytes: bytes):
    """Cached Excel loader using raw bytes to avoid repeated disk IO and re-parsing."""
    try:
...
def load_predictor_cached(model_bytes: bytes):
    """Cache the loaded predictor for identical model bytes to speed reruns."""
    vp = VoterPredictor()
    # pass BytesIO so load_model reads from memory
    ok = vp.load_model(io.BytesIO(model_bytes))
    if not ok:
        raise RuntimeError("Failed to load model from provided bytes")
    return vp
...
def display_predictions(pred):
    ...
    with col2:
        ...
        align_cat = pred.get('alignment_category', None)
        conf_label = ("Core" if align_cat == 'core' else "Leaning" if align_cat == 'leaning' else "Swing") if align_cat else ("High" if max_prob > 0.4 else "Medium" if max_prob > 0.25 else "Low")
        conf_emoji = "🎯" if max_prob > 0.4 else "🎲" if max_prob > 0.25 else "❓"

        st.metric(
            label="Most Likely Party",
            value=pred['predicted_party'],
            delta=f"{conf_emoji} {max_prob:.1%} ({conf_label})"
        )
    ...
    with c2:
        ...
        align_cat = pred.get('alignment_category', None)
        if align_cat == 'core':
            st.success(f"🎯 Core alignment to {pred['predicted_party']} ({max_prob:.1%})")
        elif align_cat == 'leaning':
            st.warning(f"🎲 Leaning towards {pred['predicted_party']} ({max_prob:.1%})")
        elif align_cat == 'swing':
            st.info("❓ Swing/uncertain voter")
        else:
            if max_prob > 0.4:
...
def main():
    ...
    with st.spinner("Loading model..."):
        try:
            predictor = load_predictor_cached(model_file.getvalue())
        except Exception as e:
    ...
    with st.spinner("Loading voter data..."):
        voter_df = load_voter_data(voter_file.getvalue())
        if voter_df is None or voter_df.empty:
            st.error("No voter data found.")
            return

    # Optional: normalize numeric columns (can be slow on very large files)
    normalize_numeric = st.sidebar.checkbox("Normalize numeric columns on load (slower)", value=False)
    if normalize_numeric:
        with st.spinner("Normalizing numeric columns..."):
            for col in [
    ...
    # Optional row limit for speed on large files
    row_limit = st.sidebar.number_input("Row limit (0 = all)", min_value=0, value=0, step=100)
    if row_limit and row_limit > 0:
        voter_df = voter_df.head(int(row_limit))
        st.sidebar.info(f"Using first {len(voter_df)} rows for analysis (row limit)")
    ...
    with st.expander("🔧 Model Debug"):
        ...
        if getattr(predictor, 'training_info', None):
            st.write("**Training Info**")
            ti = predictor.training_info
            st.write({
                "early_stopping": ti.get("early_stopping"),
                "best_epoch": ti.get("best_epoch"),
                "total_epochs": ti.get("total_epochs"),
                "max_epochs": ti.get("max_epochs"),
                "patience": ti.get("patience"),
            })

        # Alignment thresholds if available
        if getattr(predictor, 'alignment_thresholds', None):
            st.write("**Alignment thresholds**", predictor.alignment_thresholds)

        # Corrections log (preview + download) — avoid nested expanders (Streamlit restriction)
        if getattr(predictor, 'correction_log', None):
            show_log = st.checkbox("Show data correction log (preview)", value=False)
            if show_log:
                preview = predictor.correction_log[:50]
                st.write(f"Showing first {len(preview)} of {len(predictor.correction_log)} entries")
                st.json(preview)
                csv_bytes = ("\n".join(str(x) for x in predictor.correction_log)).encode("utf-8")
                st.download_button("Download full correction log", data=csv_bytes, file_name="correction_log.txt")

        # Optional: run deeper debug probes (slower)
        if st.checkbox("Run deep debug probes (slower)", value=False):
            try:
        ...
        # Detailed debug summary (opt-in)
        if st.checkbox("Show detailed model debug summary", value=False):
            try:
                sample = voter_df.head(5).to_dict('records') if len(voter_df) > 0 else []
